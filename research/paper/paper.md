...
**Limitations.** (1) The bench is deterministic and small (n=36); we use it only for qualitative attribution/invariance, not as a safety rate. (2) §5.2 materiality labels are author-defined (though independent of the entropy weights); a different labeling could favor the count. (3) §5.3 corrupts signals i.i.d.; real injection corruption is correlated and adversarial, which would likely be *worse* than this graceful curve — so §5.3 is an optimistic bound, stated as such. (4) The external AgentDojo evaluation (gate as injection defense; baseline PromptArmor <1% ASR [19]) is harness-complete but user-gated (needs a live key) and is reported as planned external validation, not evidence. (5) Separability holds only *given faithful extraction*; a compromised extractor defeats the gate as it would any gate.
