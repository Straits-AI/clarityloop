...
1. **Benchmark abstracts tools/verifiers as a deterministic ground-truth oracle.** Each `BenchmarkCase` carries explicit ground-truth flags; runners read them instead of calling `@clarityloop/tools`/`@clarityloop/verifiers`. This keeps `packages/evals` dependent only on `@clarityloop/core` + `@clarityloop/qwen`, makes every run reproducible in CI with zero network, and avoids re-testing tool/verifier internals (already covered by Plans 4–5). The ClarityLoop runner still drives a real entropy loop through the **real `scoreEntropy`** kernel and applies the design-spec §7 commit rule, so the benchmark faithfully represents ClarityLoop's governance behavior. Swapping in `runCommitGate` (core, Plan 5) for the final decision is a localized change in `runners.ts`.
