...
4. **needs_approval** — any of: risk above `autoCommitMaxRiskClass`; an **authority-boundary
   category** that always requires approval (`AuthorityBoundary.approvalRequiredFor`, surfaced as a
   `Check` whose `name` equals the category — e.g. `external_send`, `high_value_quote` — and forced
   regardless of entropy or the risk ceiling); a **blocking `evidence_coverage_threshold`** failure
   or numeric coverage below `requireApprovalIf.evidenceCoverageBelow`; a `policy` warn; residual
...
orchestrator fills those in and the summary is generated by Qwen (`audit_narrative` task). The
orchestrator (`apps/api/src/authority-checks.ts`) derives the active authority-boundary category
`Check`s from the request's `riskSignals` so the gate can enforce `approvalRequiredFor` end-to-end.

## Loop integration (`makeApprovalRequired`)

`apps/api/src/loop/approval-gate.ts` exports `makeApprovalRequired`, the production
`LoopDeps.approvalRequired` predicate: it composes the verifiers + entropy + risk + `runCommitGate`
so the next-best-action loop short-circuits to a human the moment the gate would return
`needs_approval` or an L4 `sandbox_only`. Model proposes; the deterministic gate decides.
