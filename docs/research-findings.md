...
| 4 | **Adversarial emission (trust boundary)** | Corrupting model self-report defeats **1/6** unsafe archetypes; corrupting the independent verifier layer defeats **4/6**. The guarantee reduces to **extraction integrity**. | `research/experiments/adversarial_emission.ts` |
